<template>
  <div>
    ...
    >
      <el-table-column prop="nav_id" label="首页导航ID" align="center"></el-table-column>
      <el-table-column prop="pid" label="上一级id" align="center"></el-table-column>
      <el-table-column prop="name" label="导航名称" align="center"></el-table-column>
      <el-table-column prop="link_url" label="导航跳转链接" align="center">
      ...
      <el-table-column label="操作" nim-width="180" align="center">
        <template slot-scope="scope">
          ...
          <el-tooltip class="item" effect="dark" content="删除" placement="top">
            <el-button type="danger" @click="del({'nav_id':scope.row.nav_id})">
...
<script>
import { getNavigateLists, delNavigateLists } from "@/api/user";
import NavigateModel from "./NavigateModel";
export default {
  components: { NavigateModel },
  data() {
    return {
      ediParams: null,
      tableData: [],
  ...
  methods: {
    ...
    del(params) {
      this.$confirm("删除该项, 是否继续?", "提示", {
        ...
        .then(() => {
          delNavigateLists(params).then(res => {
            this.update();
            this.$message({
              message: res.msg
            });
    ...
    update(params) {
      this.loading = true;
      getNavigateLists(params).then(res => {
        this.loading = false;
